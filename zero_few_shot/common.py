...
def _standardize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize column names for proverb datasets.

    The upstream dataset uses capitalised names ``Proverbs`` and
    ``Explanation``, whereas the code expects ``proverb`` and
    ``reference_explanation``.  This helper performs a case‑insensitive
    mapping of common synonyms to the canonical names.

    Parameters
    ----------
    df: pandas.DataFrame
        Input DataFrame whose columns will be renamed in place.

    Returns
    -------
    pandas.DataFrame
        The same DataFrame with columns renamed.
    """
    rename_map: dict[str, str] = {}
    # Build a lookup of lower‑cased column names to original names
    lower_to_orig = {c.lower(): c for c in df.columns}
    # Map possible spellings of the proverb column
    ...
    # Map possible spellings of the explanation column
...
def load_data(csv_path: str) -> pd.DataFrame:
    ...
    The CSV is expected to have at least a column named ``proverb`` (case
    insensitive).  Optionally a column named ``reference_explanation`` may be
    present; this column is used when selecting few‑shot examples.  Any other
    ...
    -------
    pandas.DataFrame
        Loaded data frame with standardized column names.
    """
    ...
    if "proverb" not in df.columns:
        raise ValueError(
            "Input CSV must contain a 'proverb' column (or 'Proverbs')"
        )
    return df
...
def select_few_shots(
    ...
    Only rows where ``reference_explanation`` is non‑empty are eligible for
    selection.  The random seed ensures reproducibility.
    ...
    k: int
        Number of examples to sample.  If zero or fewer, an empty list is
        returned.
    seed: int
